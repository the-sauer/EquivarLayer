...
def compute_RS_invariants(u):
    ...
    inv26 = normalize(torch.cat((inv2, inv6), 1))

    return torch.cat((inv1, inv345, inv26), 1)


# EquivarLayer for the rotation-scale group
...
# Computing an equivariant matrix for the rotation-scale group
class EqMatrixLayer(nn.Module):
    ...
    def forward(self, x):
        diff = Diff(ord=1)
        ux, uy = diff(x)
        eq_matrix11 = self.conv1(uy)
        eq_matrix12 = self.conv1(ux)
        return torch.cat(
                (eq_matrix11, eq_matrix12, -eq_matrix12, eq_matrix11),
                dim=1)
...
class EquivarLayer_RS_resnet32(nn.Module):
    def __init__(self,
                 in_shape,
                 beta = 10,
                 block = BasicBlock,
        ...
        super(EquivarLayer_RS_resnet32, self).__init__()
        self.beta = beta
        self.in_planes = 16
