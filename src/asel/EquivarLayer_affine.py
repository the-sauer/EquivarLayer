...
def compute_affine_invariants(u, scales=None):
    ...
    if scales is not None and u.shape[1] == 1:
        u = u.expand(-1, len(scales), -1, -1, -1)  # Expand scale dimension if scales are provided but input has only one scale, TODO: Is this correct? Shouldnt u also be scaled, consider adding a scaled u as an output of Diff
...
class EquivarLayer(nn.Module):
    def __init__(self, in_channels, out_channels, hid_channels=None, type=["0", "0"], stride=1, scales=None):
        ...
        self.type = type
        if self.type[1] == "c":
            self.eq_matrix_layer = EqMatrixLayer(in_channels, out_channels, scales=scales)
    ...
    def forward(self, x: Union[torch.Tensor, tuple[torch.Tensor, torch.Tensor]]):
        ...
        assert torch.is_tensor(x), "`x` must be a torch.Tensor or a tuple with tensor first element."
        assert x.dim() in (4, 5), "`x` must be 4D (B,C,H,W) or 5D (B,S,C,H,W)."
        assert s is None or self.scales is None
        assert x.shape[-3] == self.in_channels, f"Expected {self.in_channels} input channels, got {x.shape[-3]}."
        ...
        inv = self.conv2(inv)
        inv = inv.view(b, s_dim, -1, inv.shape[-2], inv.shape[-1])  # Reshape back to (B, S, out_channels, H', W')
        ...
        if self.type[1] == "0":
            ...
            eq_matrix = self.eq_matrix_layer((x, s) if s is not None else x).view(b, s_dim, 2, 2, h, w)
            out_matrix = torch.einsum('bsijkl,bsjnkl->bsinkl', eq_matrix, inv.view(b, s_dim, 2, 2, h, w))
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, option='A', scales=None):
        ...
        self.scales = scales
        self.conv1 = EquivarLayer(in_planes, planes, type=["0", "0"], stride=stride, scales=scales)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = EquivarLayer(planes, planes, type=["0", "0"], stride=1, scales=scales)
...
class EquivarLayer_affine_resnet32(nn.Module):
    def __init__(self,
        ...
        self.scales = scales
        self.in_planes = 16
        self.conv1 = EquivarLayer(in_shape[0], 16, type=["0", "0"], stride=1, scales=scales)
    ...
    def det_pool(self, x, s_dim=None):
        ...
        assert x.dim() == 4, "`det_pool` expects a 4D tensor shaped (B,4,H,W) or (B*S,4,H,W)."
        assert x.shape[1] == 4, "`det_pool` expects channel dimension to be 4 (flattened 2x2 matrix)."
        b, _, h, w = x.shape
        det = (x[:,0,:,:] * x[:,3,:,:] - x[:,1,:,:] * x[:,2,:,:]).abs()
